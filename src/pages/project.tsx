...
const ProjectPage = () => {
  ...
  return (
    <FadeIn className="container mb-5">
                ...
                <LeftSide className="p-4 w-50" leftAlignment={index % 2 == 0}>
                  ...
                  <StackList>
                    {project.Tags.map((tag, index) => (
                      <StackItem key={`project-tag-${tag}-${index}`}>
                        {tag}
                      </StackItem>
                    ))}
                  </StackList>
...
const CarouselImages = ({ Project }: { Project: ProjectDataType }) => {
  return (
    <ProjectImage className="w-50">
      <Carousel>
        {Project.Images &&
          Project.Images.map((i, index): any => (
            <Carousel.Item interval={10000} key={`image-${index}`}>
              <ImageOverlay
                alt={i.Name}
                ImageUrl={i.Url && i.Url}
                Width={600}
                Height={300}
              />
            </Carousel.Item>
...
const CardProject = ({
  ...
  SmallProjects: ProjectDataType[];
}) => {
  return (
    <>
      {SmallProjects.map((project, index) => (
        <ProjectContent
          key={`carded-projext-#${index}`}
          className="px-2 mt-2 mb-2 col-sm-6 col-md-4 col-lg-4"
        >
          <ProjectFolder className="p-2 d-flex flex-column justify-content-between h-100">
            <div className="d-flex justify-content-between">
              ...
              <LinkContainer className="d-flex flex-row">
                {project.RepositoryUrl && (
                  <a
                    aria-label={project.RepositoryUrl}
                    className="p-2"
            ...
            <ProjectH2 className="m-0 d-flex justify-content-start flex-nowrap align-items-center">
              [{project.Title}]
            </ProjectH2>
            <ProjectDescription>
              <p>{project.Description}</p>
            ...
            <StackList>
              {project.FeaturedTags &&
                project.FeaturedTags.map((tag, index) => (
...
const ProjectContainer = styled.div`
  direction: ${(props: { rightAlignment: boolean }) =>
    props.rightAlignment ? "rtl" : "initial"};
...
const LeftSide = styled.div`
  direction: ${(props: { leftAlignment: boolean }) =>
    props.leftAlignment ? "ltr" : "initial"};
  @media ${devicesMax.tablet} {
    width: 100% !important;
  }
`;
const TitleProjectFeature = styled.div`
  direction: ${(props: { leftAlignment: boolean }) =>
    props.leftAlignment ? "rtl" : "initial"};
  @media ${devicesMax.tablet} {
    width: 100% !important;
  }
`;
...
const Divider = styled.div`
  height: 0.5px;
  margin: ${(props: { divided: boolean }) =>
    props.divided ? "0 40% 0 0" : "0 0 0 40%"};
`;
